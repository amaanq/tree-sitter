...
/**
 * Increments the value of the column cache.
 * @param self The lexer state.
 */
...
static void ts_lexer_goto(Lexer *self, Length position, ColumnData column) {
...
static void ts_lexer__do_advance(Lexer *self, bool skip) {
  if (self->lookahead_size) {
    if (self->data.lookahead == '\n') {
      ...
      ts_lexer__set_column_cache(self, 0);
    } else {
      bool is_bom = self->current_position.bytes == 0 &&
        self->data.lookahead == BYTE_ORDER_MARK;
      if (!is_bom) ts_lexer__increment_column_cache(self);
  ...
  if (skip) {
    ...
    self->token_start_column = self->column_data;
  }
...
static void ts_lexer__mark_end(TSLexer *_self) {
  ...
  if (!ts_lexer__eof(&self->data)) {
    ...
    if (
      ...
      self->token_end_column = COLUMN_NONE;
      return;
    }
  }
  self->token_end_position = self->current_position;
  self->token_end_column = self->column_data;
}
...
static uint32_t ts_lexer__get_column(TSLexer *_self) {
  ...
  if (!self->column_data.valid) {
    ...
    ts_lexer_goto(self, start_of_col, COLUMN_NONE);
    ts_lexer__get_chunk(self);
...
void ts_lexer_set_input(Lexer *self, TSInput input) {
  self->input = input;
  ts_lexer__clear_chunk(self);
  ts_lexer_goto(self, self->current_position, self->column_data);
}
...
void ts_lexer_reset(Lexer *self, Length position, ColumnData column) {
  if (position.bytes != self->current_position.bytes) {
    ts_lexer_goto(self, position, column);
  }
}
...
void ts_lexer_finish(Lexer *self, uint32_t *lookahead_end_byte) {
  ...
  if (
    ...
    self->token_start_column = self->token_end_column;
  }
