...
impl Parser {
    ...
    #[deprecated(since = "0.21.0", note = "Use parse_utf16_le() method")]
    pub fn parse_utf16(
        &mut self,
        input: impl AsRef<[u16]>,
        old_tree: Option<&Tree>,
    ) -> Option<Tree> {
        self.parse_utf16_le(input, old_tree)
    }

    #[deprecated(since = "0.21.0", note = "Use parse_utf16_le_with() method")]
    ...
    ///   created, then you must edit `old_tree` to match the new text using
    ///   [Tree::edit].
    pub fn parse_utf16_le_with<'a, T: AsRef<[u16]>, F: FnMut(usize, Point) -> T>(
        &mut self,
        callback: &mut F,
        old_tree: Option<&Tree>,
    ) -> Option<Tree> {
        ...
        let c_input = ffi::TSInput {
            payload: &mut payload as *mut (&mut F, Option<T>) as *mut c_void,
            read: Some(read::<T, F>),
            encoding: ffi::TSInputEncoding_TSInputEncodingUTF16LE,
        };
    ...
    ///   created, then you must edit `old_tree` to match the new text using
    ///   [Tree::edit].
    pub fn parse_utf16_be_with<'a, T: AsRef<[u16]>, F: FnMut(usize, Point) -> T>(
        &mut self,
        callback: &mut F,
        old_tree: Option<&Tree>,
    ) -> Option<Tree> {
        ...
        let c_input = ffi::TSInput {
            payload: &mut payload as *mut (&mut F, Option<T>) as *mut c_void,
            read: Some(read::<T, F>),
            encoding: ffi::TSInputEncoding_TSInputEncodingUTF16BE,
        };
...
impl<'a> TreeCursor<'a> {
    ...
    pub fn reset<'b>(&mut self, node: Node<'b>) {
        unsafe {
            std::mem::transmute::<PhantomData<&'a ()>, PhantomData<&'b ()>>(self.1);
